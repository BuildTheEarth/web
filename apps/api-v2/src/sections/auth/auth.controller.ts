...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description:
      "Generates an API access token for a BuildTeam using its ID and token.",
  })
  ...
  generateAccessToken(@Body() generateAccessTokenDto: GenerateAccessTokenDto) {
    return this.authService.generateAccessToken(
      generateAccessTokenDto.buildTeamId,
      generateAccessTokenDto.token,
    );
  }
  ...
  @ApiOperation({
    ...
    description:
      "Verifies the API access token and returns the BuildTeam profile.",
  })
  @ApiBearerAuth()
