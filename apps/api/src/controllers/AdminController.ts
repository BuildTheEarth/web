...
class AdminController {
	...
	public async getClaimBuildingCounts(req: Request, res: Response) {
		if (this.progress.buildings.done > 0) {
			return ERROR_GENERIC(req, res, 409, 'Recalculations are already ongoing.')
		}
		...
		for (const [i, claim] of claims.entries()) {
			this.core.getWeb().getControllers().claim.updateClaimBuildingCount(claim, true)
	...
	public async getClaimOSMDetails(req: Request, res: Response) {
		...
		for (const [i, claim] of claims.entries()) {
			if (req.query?.skipOld === 'true' && claim.osmName !== '' && claim.city != '') {
				continue
			}

			await this.core.getWeb().getControllers().claim.updateClaimOSMDetails(claim, true)
	...
	public async getImageHashes(req: Request, res: Response) {
		...
		const hashes = await Promise.all(
			images.map((image) => getHash(`https://cdn.buildtheearth.net/uploads/${image.name}`)),
		)
