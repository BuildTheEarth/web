...
class GeneralController {
	...
	public async getAccount(req: Request, res: Response) {
		...
		const { exp, iat, sub, email_verified, preferred_username, email } = req.kauth.grant.access_token.content
		const user = await this.core.getPrisma().user.findFirst({ where: { ssoId: sub } })

		if (!user) return ERROR_GENERIC(req, res, 500, 'Unidentified User.')

		const userPermissions = await this.core.getPrisma().userPermission.findMany({
			where: { user: user },
			include: {
				user: false,
				permission: true,
