...
export function useCoordinateInput(coordinates: string, required?: boolean) {
	return async (req: Request, res: Response, next: NextFunction) => {
		...
		const coordType = (req.query.coordType as string) || CoordinateType.STRING_ARRAY

		req.body[coordinates + '__old'] = req.body[coordinates]

		if (!coords) {
			if (required) {
				return ERROR_VALIDATION(req, res, [{ msg: 'Invalid Value', param: coordinates, location: 'body' }])
			}
			next()
		} else {
			try {
				...
				return ERROR_GENERIC(req, res, 500, 'Error parsing coordinates. Correct type?')
			}
		}
	}
}
...
export function parseCoordinates(coords: any, type: string) {
	switch (type) {
		...
		case CoordinateType.STRING_REVERSE: {
			return coords.split(';').map((c) => `${c.split(',')[1].trim()}, ${c.split(',')[0].trim()}`)
		}
		...
		case CoordinateType.STRING_ARRAY_REVERSE: {
			return coords.map((c) => `${c.split(',')[1].trim()}, ${c.split(',')[0].trim()}`)
		}
		// [["lng","lat"],["lng","lat"],["lng","lat"]]
