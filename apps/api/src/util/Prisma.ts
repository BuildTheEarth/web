...
export async function purgeClaims(core: Core) {
	...
	const handle = async () => {
		...
		if (emptyClaims.count > 0) core.getLogger().info(`Purged ${emptyClaims.count} emtpy Claims`)

		const noRefClaims = await prisma.claim.deleteMany({
			where: { externalId: null, ownerId: null },
		})

		if (noRefClaims.count > 0) core.getLogger().info(`Purged ${noRefClaims.count} unreferenced Claims`)
	}
			...
			const content = apps?.map(
				(app) =>
					`- ${new Date(app.createdAt).toLocaleDateString('en-us', {
						...
						month: 'numeric',
						day: 'numeric',
					})}: <@${app.user.discordId}> (${app.user.minecraft}) Review it [here](${process.env.FRONTEND_URL}/teams/${
						app.buildteam.slug
