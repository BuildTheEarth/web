...
import { retryAllFailedJobs, retryFailedJob, removeJob } from '@/actions/worker'
import { toHumanDateTime } from '@/util/date'
import { ActionIcon, Badge, Button, Code, Group, Tooltip } from '@mantine/core'
...
export default function JobsDatatable({
	...
	const handleRetry = async (jobId: string) => {
		...
		try {
			...
			notifications.show({
				title: 'Job Retried',
				message: `Job ${jobId} has been re-queued for execution.`,
				color: 'green',
			})
		} catch (e: any) {
			notifications.show({
	...
	const handleRetryAll = async () => {
		...
		try {
			...
			notifications.show({
				title: 'Retrying All Failed Jobs',
				message: `${count} failed job(s) have been re-queued.`,
				color: 'green',
			})
		} catch (e: any) {
			notifications.show({
				title: 'Error',
				message: e.message || 'Failed to retry jobs',
				color: 'red',
			})
		} finally {
